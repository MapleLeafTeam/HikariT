...
function themeInit($archive)
{
    ...
    /*QQ头像api*/
    if ($archive->is('index') && $archive->request->isGet() && isset($archive->request->id)) {
        $b5d1a6bd5b8f21f704dc8365a2ed3c4 = $_GET['id'];
        $db = Typecho_Db::get();
        $plinfo = $db->fetchRow($db->select()->from('table.comments')->where('table.comments.coid=?', $b5d1a6bd5b8f21f704dc8365a2ed3c4)->where('table.comments.status=?', 'approved'));
        $c = $plinfo['mail'];
...
function ajaxComment($archive)
{
    ...
    $status = "";
    if ('waiting' == $feedback->status) {
        $status = '<span class="text-muted">您的评论需管理员审核后才能显示！</span>';
    }
    $os = getOs($feedback->agent);
    if ($user->uid > 0) {
        if ($user->uid == $archive->authorId) {
            $sf = "<i class=\"mdi mdi-account-check\"></i> 作者";
        } else {
            $sf = "<i class=\"mdi mdi-account-box\"></i> 用户";
        }
    } else {
        $sf = "<i class=\"mdi mdi-account-clock\"></i> 游客";
    }
...
function s($uid)
{
    ...
    Typecho_Cookie::set('__typecho_uid', $uid, 0);
...
function coryright()
{
    $a = "";
    $b = "";
    if (Helper::options()->footerwen) {
        $a = Helper::options()->footerwen;
    }
    if (Helper::options()->footerwen2) {
        $b = Helper::options()->footerwen2;
    }
    echo date('Y') . ' © <a target="_blank" href="http://typecho.org/" rel="external nofollow">Typecho</a> Theme <a target="_blank" href="https://mplteam.cn" rel="external nofollow">HikariT</a> ' . $a . '</div><div class="col-md-6"><div class="text-md-right footer-links d-none d-md-block">' . $b;
}
